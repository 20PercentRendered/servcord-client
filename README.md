# ServCord Client
A Discord "client" that applies hosts overrides for the discord executable. 
Primarily intended for ServCord.
Works on Windows and Linux.

# IMPORTANT INFO
By connecting to a server, the client treats it as an official server, and checks for updates.
These updates can contain malicious code. Only connect to servers you trust. 
### NEVER RUN THIS WITH ADMINISTRATOR OR ROOT PRIVILEGES.
## Install Instructions (Windows)
1. Download and install Discord.
2. Download and install the ServCord Client.

## Install Instructions (Linux)
1. Download and install Discord so it is accessible anywhere using `discord`
2. Download and install the ServCord Client.

## Usage
Type the ip you want to connect to into the IP Address field and hit `Start Discord`

Alternatively, start it with `--server IP.HERE` to directly connect to a server.

## Building
1. Clone the repo
2. `npm i`
3. `npm start`
